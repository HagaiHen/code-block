...
const Home = () => {
    const { loading, codeBlocks, setCodeBlocks } = useGetCodeBlocks();
    ...
    const handleUpdate = async () => {
        ...
        try {
          ...
          setCodeBlocks(codeBlocks.map(cb => 
    ...
    return (
        ...
        {currCodeBlock && (
          <div>
            <h5>Click for start edit</h5>
            {currCodeBlock.mentorId !== authUser._id && editMode ? (
              <div style={{"flexDirection": 'column'}}>
                <textarea
                  value={currCodeBlock.code}
                  onChange={(e) => setCurrCodeBlock({ ...currCodeBlock, code: e.target.value })}
                  rows="20"
                  cols="150"
